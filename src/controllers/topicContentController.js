...
export const createTopicContent = async (req, res) => {
    try {
        const { topic, title, number, isLocked } = req.body;

        if (!topic || !title || number === undefined) {
            return sendResponse(res, 400, "Topic, title, and number are required.");
        }

        const newContent = await TopicContent.create({ topic, title, number, isLocked });
